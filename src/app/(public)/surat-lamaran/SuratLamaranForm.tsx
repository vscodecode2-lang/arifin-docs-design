...
import { useLamaranForm } from "./hooks/useLamaranForm";
...
export function SuratLamaranForm() {
  const {
    ...
    <div className="min-h-screen bg-slate-50 py-10">
      <div className="mx-auto max-w-2xl px-4 sm:px-6">
        ...
        <div className="mb-8 text-center">
          ...
          <h1 className="text-2xl font-black text-slate-900">Formulir Pembuatan Surat Lamaran</h1>
          <p className="mt-1 text-sm text-slate-500">Isi data dengan benar untuk surat lamaran yang personal & profesional</p>
        </div>
          ...
          {STEPS.map((s, i) => {
            ...
            return (
              <div key={s.id} className="flex flex-1 items-center">
                ...
                {i < STEPS.length - 1 && (
                  <div className={`mx-1 h-0.5 flex-1 transition-colors ${step > s.id ? "bg-blue-600" : "bg-slate-200"}`} />
                )}
              </div>
            );
          })}
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
          ...
          {step === 1 && <Step1 data={form.step1} errors={errors} onChange={updateStep1} />}
          {step === 2 && <Step2 data={form.step2} errors={errors} onChange={updateStep2} />}
          {step === 3 && (
            <Step3 data={form.step3} errors={errors}
              careerStatus={form.step1.career_status} onChange={updateStep3} />
          )}
          {step === 4 && (
            <Step4Review form={form} invalidSteps={invalidSteps} onGoToStep={goToStep} />
          )}
        ...
        <div className="mt-5 flex items-center justify-between">
          ...
          {step < STEPS.length ? (
            ...
            <button onClick={handleSubmit} disabled={isLoading}
              className="flex items-center gap-2 rounded-xl bg-blue-700 px-6 py-2.5 text-sm font-bold text-white shadow-sm hover:bg-blue-800 active:scale-95 disabled:cursor-not-allowed disabled:opacity-60 transition-all">
              {isLoading
        ...
        <p className="mt-4 text-center text-xs text-slate-400">
          🔒 Data Anda disimpan dengan aman dan hanya digunakan untuk keperluan pembuatan surat lamaran.
        </p>
      </div>
    </div>
  );
